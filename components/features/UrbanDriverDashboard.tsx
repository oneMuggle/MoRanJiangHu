...
const TripInfo: React.FC<TripInfoProps> = ({ 当前行程类型, 当前地点, 行车记录仪状态 }) => {
  if (!当前行程类型) return null;
  ...
  return (
    <div className="rounded border border-white/10 bg-gray-900/50 p-3">
      <div className="flex items-center gap-3 text-sm">
        <span className="text-lg">{行程类型图标[当前行程类型] || '🚗'}</span>
        <span className="text-gray-200 font-medium">{当前行程类型}</span>
        {当前地点 && (
          <span className="text-gray-500 text-xs">→ {当前地点}</span>
        )}
      </div>
      <div className="mt-2 flex gap-4 text-xs">
...
const UrbanDriverDashboard: React.FC<UrbanDriverDashboardProps> = ({ 都市网约车系统, onClose }) => {
  ...
  const hasData = Object.keys(乘客欲望档案).length > 0 || 后果列表.length > 0 || 行程系统?.当前行程类型;
