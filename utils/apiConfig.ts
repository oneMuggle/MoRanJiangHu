...
export const 获取文生图接口配置 = (settings: 接口设置结构): 当前可用接口结构 | null => {
    ...
    if (Array.isArray(imgConfigs) && imgConfigs.length > 0) {
        ...
        const resolvedBaseUrl = isCnbComfyui
            ? (cnbBaseUrl || baseUrl)
            : (baseUrl || (backend === 'openai' || backend === 'novelai' || backend === 'grok' ? current.baseUrl : ''));
    ...
    const 图片后端类型 = feature?.文生图后端类型 === 'novelai' || feature?.文生图后端类型 === 'sd_webui' || feature?.文生图后端类型 === 'comfyui' || feature?.文生图后端类型 === 'grok'
        ? feature.文生图后端类型
        : 'openai';

    if (!enabled) return null;
    const imageModel = 读取字符串(feature?.文生图模型使用模型).trim();
    const 图片后端需要模型 = 图片后端类型 === 'openai' || 图片后端类型 === 'novelai' || 图片后端类型 === 'grok';
    ...
    const 图片后端可复用主接口地址 = 图片后端类型 === 'openai' || 图片后端类型 === 'novelai' || 图片后端类型 === 'grok';
    ...
    const 图片后端需要鉴权 = 图片后端类型 === 'openai' || 图片后端类型 === 'novelai' || 图片后端类型 === 'grok';
...
return {
    ...
    const sceneBackend: NonNullable<当前可用接口结构['图片后端类型']> = feature?.场景生图后端类型 === 'novelai' || feature?.场景生图后端类型 === 'sd_webui' || feature?.场景生图后端类型 === 'comfyui' || feature?.场景生图后端类型 === 'grok'
        ? feature.场景生图后端类型
        : 'openai';
    const sharedBackend: NonNullable<当前可用接口结构['图片后端类型']> = sharedConfig.图片后端类型 === 'novelai' || sharedConfig.图片后端类型 === 'sd_webui' || sharedConfig.图片后端类型 === 'comfyui' || sharedConfig.图片后端类型 === 'grok'
        ? sharedConfig.图片后端类型
        : 'openai';
    const sceneModel = 读取字符串(feature?.场景生图模型使用模型).trim();
    const 场景后端需要模型 = sceneBackend === 'openai' || sceneBackend === 'novelai' || sceneBackend === 'grok';
    ...
    const 场景后端需要鉴权 = sceneBackend === 'openai' || sceneBackend === 'novelai' || sceneBackend === 'grok';
    ...
    const scenePresetPathMap: Record<'openai' | 'novelai' | 'sd_webui' | 'comfyui' | 'grok', NonNullable<当前可用接口结构['图片预设接口路径']>> = {
